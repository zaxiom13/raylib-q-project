...
/ --- Test Group 14: Legacy binding surface ---
legacyFns:.raylib.api._bindings;
assertEq["legacy binding count";count legacyFns;159];
assertEq["legacy usage count";count key .raylib.api._usage;159];
docInitLegacy:.raylib.help[`InitWindow];
assertEq["legacy help has usage";docInitLegacy like "*usage: .raylib.InitWindow*";1b];
errLegacyArgs:.raylib.api.call[`InitWindow;()];
assertEq["legacy wrong argc uses usage";errLegacyArgs;.raylib.api._usage`InitWindow];
lineStripTbl:([] points:enlist (0 0f;10 10f); color:enlist .raylib.Color.BLACK);
lineStripResult:.[.raylib.lineStrip;enlist lineStripTbl;{x}];
assertEq["lineStrip table-first callable";10h=type lineStripResult;0b];
docLineStrip:.raylib.help[`lineStrip];
assertEq["lineStrip help usage";docLineStrip like "*.raylib.lineStrip*";1b];
origEscKey:.raylib.interactive._escKey;
.raylib.HideCursor[];
assertEq["legacy hide cursor updates state";.raylib.IsCursorHidden[];1b];
.raylib.ShowCursor[];
assertEq["legacy show cursor updates state";.raylib.IsCursorHidden[];0b];
.raylib.SetTraceLogLevel 5;
assertEq["legacy trace log state set";.raylib.api._state`traceLog;5i];
.raylib.SetExitKey 123;
assertEq["legacy exit key state set";.raylib.api._state`exitKey;123i];
assertEq["legacy exit key interactive esc set";.raylib.interactive._escKey;123i];
.raylib.interactive._escKey:origEscKey;
i:0;
while[i<count legacyFns;
  nm:legacyFns i;
  sym:`$".raylib.",string nm;
  fn:.[value;enlist sym;{`missing}];
  if[`missing~fn;
    failures,:enlist raze ("legacy missing fn ";string nm);
   ;
    if[not .raylib._isCallable fn;
      failures,:enlist raze ("legacy not callable ";string nm);
     ;
      argc:.raylib.api._argCount nm;
      args:$[argc=0;();argc#enlist 0];
      r:.[fn;args;{`invokeError}];
      if[`invokeError~r;
        failures,:enlist raze ("legacy invoke error ";string nm)]]];
  i+:1];
